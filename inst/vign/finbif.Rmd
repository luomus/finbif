...
partial matching.
```{r taxon-search, eval = FALSE}
birch_search <- finbif_taxa("Betula pendula", 2, "partial")
birch_search$content
```
```{r taxon-search-eval, echo = FALSE, cache = TRUE}
birch_search <- finbif_taxa("Betula pendula", 2, "partial")
details::details(birch_search$content, "Click to toggle output.", NULL)
```
...
```
```{r occurrence2-eval, echo = FALSE, cache = TRUE}
details::details(finbif_occurrence(
  "Cygnus cygnus", 
  "Cygnus olor",
  filter = list(coordinates_uncertainty_max = 100)
...
```{r plot-occurrences-eval, echo = FALSE, fig.cap = "", fig.width = 5, fig.height = 7, cache = TRUE}
# Download all the occurrences of Eurasian Jay in Finland
# that have coordinates accurate to at least 100m
jays <- finbif_occurrence(
  taxa   = "Eurasian Jay",
