...
microorganisms. The [finbif](https://github.com/luomus/finbif) R package
...
[finbif](https://github.com/luomus/finbif) from
...
If the taxon is in the FinBIF database its unique ID is returned. When a
taxon is not in the FinBIF database it is reported as “not found” and
...
You can also specify the taxonomic rank when searching FinBIF and the
...
matching, but multiple taxa, up to a limit, `n`, may be returned when
...
It is possible to request a random sample of records instead of the last
