...
Much of the information in the FinBIF database consists of metadata that
helps provide context for occurrence records and other information in
FinBIF.

## General metadata

You can see some of the metadata available in
[finbif](https://github.com/luomus/finbif) by calling the
...
Some more complex metadata is accessed with other
[finbif](https://github.com/luomus/finbif) functions
...
You can select a subgroup by specifying a parent informal group as a
function argument.
...
You can subset collection metadata by using the `filter` and `select`
arguments.
...
You can see the child collections of a supercollection by specifying the
ID as a filter. Note that the children of supercollections may also be
supercollections.
